...
// or 3x3 grid) and print/dump the resulting mesh — the VM twin of
// tools/isolate_group_mesh.py, for per-group Blender diffing.
// Usage: npx tsx tools/gnvm-isolate.ts <dump.json> <GROUP> [quad|grid] [out.json]
...
const [, , dumpPath, groupName, shape = "quad", outPath] = process.argv;
...
// Bind interface defaults; the geometry input gets the test mesh.
...
for (const item of g.interface) {
  ...
  else if (item.socket_type === "NodeSocketVector") bindings[item.identifier] = Field.of((item.default ?? [0, 0, 0]) as Vec3);
  else bindings[item.identifier] = Field.of(typeof item.default === "number" ? item.default : item.default ? 1 : 0);
}
