...
1. **Chrome Asset Library** — 43 active objects, 26 modifier roots, 404 groups. Eleven root families have browser evidence, with Chain and Mace now surface-exact and UI Window Generator near-exact; continue in increasing reachable-node complexity. `soft pixel marker.001` is source-blocked by the missing Budokan font; `Dojo_STRING TO TEXT` is extracted with Blender evidence but remains pending exact Bfont outline/layout parity.
