...
1. **Chrome Asset Library** — 43 active objects, 26 modifier roots, 404 groups. Fifteen root families have browser evidence, including both Pixel Marker users, Soft Pixel Marker surface parity, the exact non-surface Mesh to Curve helper, Chain and Mace surface parity, the near-exact UI Window Generator, and editable 3D String to Text; continue in increasing reachable-node complexity. `soft pixel marker.001` is source-blocked by the missing Budokan font; `Dojo_STRING TO TEXT` is extracted with Blender evidence but remains pending exact Bfont outline/layout parity.
