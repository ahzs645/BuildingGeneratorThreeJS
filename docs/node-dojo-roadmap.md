...
1. **Chrome Asset Library** — 43 active objects, 26 modifier roots, 404 groups. Twelve root families have browser evidence, including the exact non-surface Mesh to Curve helper, Chain and Mace surface parity, and the near-exact UI Window Generator; continue in increasing reachable-node complexity. `soft pixel marker.001` is source-blocked by the missing Budokan font; `Dojo_STRING TO TEXT` is extracted with Blender evidence but remains pending exact Bfont outline/layout parity.
