...
1. **Chrome Asset Library** — 43 active objects, 26 modifier roots, 404 groups. Twenty-one root families have browser evidence, including both Pixel Marker users, both Soft Pixel Marker roots, Text Soup and Blunt Outline Crayon surface parity, the exact non-surface Mesh to Curve helper, Chain and Mace surface parity, the near-exact UI Window Generator, both editable String to Text roots, and the exact authored Chrome Tools text plate; continue in increasing reachable-node complexity.
