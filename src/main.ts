...
export function createTool(): BuildingToolHandle {
  ...
  return {
    ...
    dispose(): void {
      ...
      // UI: lil-gui panel (on body), controls, floating overlays
      gui.destroy();
      controls.dispose();
      tip.remove();
      fpsEl.remove();

      // dev hooks (would otherwise capture this whole runtime forever)
      delete devWindow.__setParams;
      delete devWindow.__setCamera;
      delete devWindow.__shot;
      delete devWindow.__setEnv;
      delete devWindow.__debug;
      delete devWindow.__snow;

      // GPU resources: everything in the scene graph plus the debug caches
      const seenMaterials = new Set<Material>();
      const disposeMaterial = (m: Material): void => {
      ...
      scene.traverse(o => {
        const obj = o as unknown as { geometry?: BufferGeometry; material?: Material | Material[] };
      ...
      edgesCache.clear();
      albedoCache.forEach(disposeMaterial);
      albedoCache.clear();
      disposeMaterial(normalViewMat);
      building = null;
