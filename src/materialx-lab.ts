...
const canvas = required<HTMLCanvasElement>("#materialx-canvas");
const backendSelect = required<HTMLSelectElement>("#materialx-backend");
const variantSelect = required<HTMLSelectElement>("#materialx-variant");
const status = required<HTMLElement>("#materialx-status");
const rendererStatus = required<HTMLElement>("#materialx-renderer");
const graphStatus = required<HTMLElement>("#materialx-graph");
const fallbackStatus = required<HTMLElement>("#materialx-fallback");
...
const probe = new THREE.Mesh(makeProbeGeometry(), normalizedMaterial());
probe.rotation.y = -0.38;
scene.add(probe);
const floor = new THREE.Mesh(
  new THREE.CircleGeometry(3.4, 96),
  new THREE.MeshPhysicalMaterial({ color: 0x252a2d, roughness: 0.82, metalness: 0 }),
);
...
function applySelection(): void {
  ...
  document.documentElement.dataset.materialxReady = "true";
  document.documentElement.dataset.materialBackend = resolution.resolved;
}
...
async function start(): Promise<void> {
  ...
  const radiance = prepareMaterialXRadiance(
  ...
  if (coordinateDiagnostic && officialEssl) {
    ...
    const card = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), createCoordinateDiagnosticMaterial(radiance, lightData));
    ...
    document.documentElement.dataset.materialxReady = "true";
    document.documentElement.dataset.materialBackend = "materialx";
    document.documentElement.dataset.materialxImplementation = implementation;
  ...
  try {
    const [normalMap, roughnessMap] = await Promise.all([
      new THREE.TextureLoader().loadAsync(publicUrl("materialx/baked/chrome-crayon-noise-normal.png")),
      new THREE.TextureLoader().loadAsync(publicUrl("materialx/baked/chrome-crayon-roughness.png")),
    ]);
    normalMap.colorSpace = THREE.NoColorSpace;
    roughnessMap.colorSpace = THREE.NoColorSpace;
    bakedPbrMaterial = new THREE.MeshPhysicalMaterial({
      color: 0xcccccc,
      metalness: 1,
      roughness: 1,
      normalMap,
  ...
  const xml = await fetch(publicUrl("materialx/chrome-crayon-prototype.mtlx"), { cache: "no-store" }).then((response) => {
  ...
  if (officialEssl) {
    ...
    const manifest = await fetch(`${generatedBase}/manifest.json`, { cache: "no-store" }).then((response) => {
    ...
    const entries = await Promise.all([
      ...
      shaderName,
      await createMaterialXEsslMaterial({
    ...
    if (uiNormalBandDiagnostic) {
      ...
      const [uiManifest, uiXml, uiReport] = await Promise.all([
        fetch(`${uiBase}/manifest.json`, { cache: "no-store" }).then((response) => {
          if (!response.ok) throw new Error(`UI normal-band manifest fetch failed: ${response.status}`);
          return response.json() as Promise<EsslManifest>;
        }),
        fetch(publicUrl("materialx/ui-normal-band-prototype.mtlx"), { cache: "no-store" }).then((response) => {
          if (!response.ok) throw new Error(`UI normal-band MaterialX fetch failed: ${response.status}`);
          return response.text();
        }),
        fetch(publicUrl("materialx/ui-normal-band.report.json"), { cache: "no-store" }).then((response) => {
      ...
      materialXMaterials.UiNormalBandSemanticRecovery = await createMaterialXEsslMaterial({
        baseUrl: uiBase,
        manifest: uiManifest,
  ...
  if (selectedLightDiagnostic && officialEssl) {
    ...
    if (threeLightDiagnostic) {
      diagnosticMaterial = new THREE.MeshPhysicalMaterial({ color: 0xcccccc, metalness: 1, roughness: 0.32 });
