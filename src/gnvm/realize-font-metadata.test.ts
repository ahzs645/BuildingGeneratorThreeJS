...
test("Realize Instances drops extraction-only font sampling metadata", () => {
  ...
  glyph.curveAttributes.set("__gnvm_planar_font_curve", { domain: "CURVE", data: [1] });
  ...
  assert.deepEqual(realized.curveAttributes.get("__gnvm_planar_font_curve"), { domain: "CURVE", data: [1] });
