...
export function createTool(): ToolHandle {
  ...
  let dump: Dump | undefined = dumpCache;
  let runId = 0;
  let appliedId = 0;
  let editTimer = 0;
  let disposed = false;
  ...
  function evaluate(): Promise<WorkerReply & { ok: true }> {
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = (event: MessageEvent<WorkerReply>) => {
        worker.terminate(); activeWorkers.delete(worker);
        if (!event.data.ok) reject(new Error(event.data.error)); else resolve(event.data);
      };
      worker.onerror = (event) => { worker.terminate(); activeWorkers.delete(worker); reject(new Error(event.message)); };
  ...
  const onReset = (): void => { distanceInput.value = "2.151417"; sizeInput.value = "1"; queueUpdate(); };
  ...
  if (dump) {
    ...
    fetch(publicUrl("dojo/periodic-brush/dump.json"))
      .then((response) => response.json())
      .then((loaded: Dump) => { dumpCache = loaded; dump = loaded; if (!disposed) void update(); })
  ...
  return {
    dispose(): void {
      ...
      distanceInput.removeEventListener("input", queueUpdate);
      sizeInput.removeEventListener("input", queueUpdate);
      ...
      delete (window as typeof window & { __PERIODIC_BRUSH__?: unknown }).__PERIODIC_BRUSH__;
    },
  };
}
