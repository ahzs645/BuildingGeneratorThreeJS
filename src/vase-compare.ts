...
export function createTool(): ToolHandle {
  ...
  reframeButton.addEventListener("click", () => frameAll(), { signal });
  ...
  if (params.get("wire") === "0" || params.get("solid") === "1") {
    vmMode = "solid";
    timeouts.push(setTimeout(applyVmMode, 500));
    timeouts.push(setTimeout(applyVmMode, 2000));
  }
  syncComparison(false);
  ...
  return {
    dispose() {
      ...
      abort.abort(); // removes every window/button listener added above
