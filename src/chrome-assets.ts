...
function frame(): void { const box = new THREE.Box3().setFromObject(model); if (box.isEmpty()) return; const center = box.getCenter(new THREE.Vector3()), size = box.getSize(new THREE.Vector3()), radius = Math.max(size.length() * .5, 1); const halfFov=THREE.MathUtils.degToRad(camera.fov*.5); const distance=(radius/Math.sin(halfFov))*1.18; const direction=new THREE.Vector3(0,-1.6,1.1).normalize(); camera.position.copy(center).addScaledVector(direction,distance); camera.near=radius/300;camera.far=radius*100;camera.updateProjectionMatrix();orbit.target.copy(center);orbit.update(); }
