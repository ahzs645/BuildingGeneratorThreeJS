...
type Asset = { id: string; title: string; object: string; dump: string; reference: string; blenderStats: { verts: number; faces: number }; note?: string; font?: AssetFont; controls: Control[] };
...
async function choose(): Promise<void> { current=catalog.find((item)=>item.id===select.value)??catalog[0];const asset=current;reference.src=publicUrl(current.reference);blenderCount.textContent=`${current.blenderStats.verts.toLocaleString()} verts · ${current.blenderStats.faces.toLocaleString()} faces`;renderControls();await prepareFont(asset);if(current!==asset)return;dump=await fetch(publicUrl(current.dump),{cache:"no-store"}).then((response)=>response.json());await evaluate(); }
