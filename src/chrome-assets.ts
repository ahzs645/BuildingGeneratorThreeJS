...
type Asset = { id: string; title: string; object: string; dump: string; reference: string; blenderStats: { verts: number; faces: number }; note?: string; font?: AssetFont; flatShading?: boolean; localSpace?: boolean; surfaceBounds?: boolean; controls: Control[] };
...
function makeMesh(soup: TriSoup): THREE.Mesh { const geometry=new THREE.BufferGeometry();geometry.setAttribute("position",new THREE.BufferAttribute(soup.positions,3));geometry.setAttribute("normal",new THREE.BufferAttribute(soup.normals,3));geometry.setIndex(new THREE.BufferAttribute(soup.indices,1));if(current.surfaceBounds&&soup.indices.length){const bounds=new THREE.Box3();const point=new THREE.Vector3();for(const index of soup.indices){const offset=index*3;point.set(soup.positions[offset],soup.positions[offset+1],soup.positions[offset+2]);bounds.expandByPoint(point);}geometry.boundingBox=bounds;}const meshMaterial=material.clone();meshMaterial.flatShading=current.flatShading??false;const mesh=new THREE.Mesh(geometry,meshMaterial);const source=(dump.objects as any[]|undefined)?.find((object)=>object.name===current.object);if(!current.localSpace&&source?.rotation)mesh.rotation.set(Number(source.rotation[0]??0),Number(source.rotation[1]??0),Number(source.rotation[2]??0));if(!current.localSpace&&source?.scale)mesh.scale.set(Number(source.scale[0]??1),Number(source.scale[1]??1),Number(source.scale[2]??1));return mesh; }
