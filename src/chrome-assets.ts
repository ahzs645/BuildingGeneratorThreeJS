...
// Pure cross-mount cache: loaded FontFaces stay registered on document.fonts,
// so remounts skip the network round trip. Safe to persist across dispose().
...
export function createTool(): ToolHandle {
  ...
  if (authoredCapture) {
    ...
    pmremEnvironment = pmrem.fromScene(room, .04).texture;
    scene.environment = pmremEnvironment;
  ...
  function frame(): void { const linePreview=document.querySelector<HTMLSelectElement>('[data-control="__linePreview"]')?.value;const boundsMode=resolveObjectBoundsMode({boundsMode:current?.boundsMode,surfaceBounds:current?.surfaceBounds,looseLinesVisible:linePreview!=="hide"});const bounds=preciseObjectBounds(model,boundsMode);if (bounds.isEmpty()) return; const viewport=canvas.getBoundingClientRect();const aspect=viewport.width/Math.max(viewport.height,1);const center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3()),radius=Math.max(size.length()*.5,.5);const halfWidth=Math.max(size.x,size.y,size.z,1e-4)*.725;camera.left=-halfWidth;camera.right=halfWidth;camera.top=halfWidth/Math.max(aspect,1e-6);camera.bottom=-camera.top;const direction=new THREE.Vector3(1,-1.25,.85).normalize();camera.position.copy(center).addScaledVector(direction,radius*3);camera.up.set(0,0,1);camera.lookAt(center);camera.near=radius/300;camera.far=radius*100;camera.updateProjectionMatrix();if(authoredKey&&authoredFill){authoredKey.width=authoredKey.height=radius*1.5;authoredKey.position.copy(center).addScaledVector(new THREE.Vector3(-1.8,-2.1,2.8).normalize(),radius*2.4);authoredKey.lookAt(center);authoredFill.width=authoredFill.height=radius*2;authoredFill.position.copy(center).addScaledVector(new THREE.Vector3(2,1,1).normalize(),radius*2);authoredFill.lookAt(center);}orbit.target.copy(center);orbit.update(); }
  // "__"-prefixed controls are viewer-only (__materialPreview, __linePreview)
  // and never reach the worker — except __frame, which is a real GN-VM
  // override that selects the evaluated animation frame.
  function overrides(): Record<string, number | boolean | string | number[]> { const values: Record<string, number | boolean | string | number[]> = {}; for (const control of current.controls) { if(control.name.startsWith("__")&&control.name!=="__frame")continue;const requested=captureOverrideValue(control.value,query.get(`override.${control.name}`));if(requested!==undefined){values[control.name]=requested;continue;}const input=document.querySelector<HTMLInputElement|HTMLSelectElement>(`[data-control="${control.name}"]`); values[control.name]=control.type==="checkbox"?((input as HTMLInputElement|null)?.checked??control.value):control.type==="text"?(input?.value??control.value):control.type==="select"?(typeof control.value==="number"?Number(input?.value??control.value):(input?.value??control.value)):control.type==="vector"?Array.from(document.querySelectorAll<HTMLInputElement>(`[data-control="${control.name}"]`)).sort((a,b)=>Number(a.dataset.axis)-Number(b.dataset.axis)).map((item,index)=>Number(item.value??control.value[index])):rangeOverrideValue(control.value,input?.value,input?.dataset.dirty==="true"); } return values; }
  ...
  async function prepareFont(asset: Asset): Promise<void> {
    ...
    const loaded=await loading;if(disposed||current!==asset)return;
  ...
  async function evaluate(): Promise<void> {
    ...
    if (disposed || result.id < appliedId || id !== requestId) return;
  ...
  return {
    dispose(): void {
      ...
      orbit.dispose();
      temporalCapture?.dispose();
      for (const child of [...model.children]) disposeObject(child);
      model.clear();
      material.dispose();
      pmremEnvironment?.dispose();
      scene.environment = null;
