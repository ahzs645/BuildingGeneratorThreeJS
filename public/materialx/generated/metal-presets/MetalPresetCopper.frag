...
uniform float surface_copper_opacity;
uniform bool surface_copper_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_copper_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_copper_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            surface_copper_out.color += lightShader.intensity * conductor_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_copper_out.color += occlusion * conductor_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
        surface_copper_out.color += conductor_out.response;

        // Compute and apply surface opacity
        {
            surface_copper_out.color *= surfaceOpacity;
            surface_copper_out.transparency = mix(vec3(1.0), surface_copper_out.transparency, surfaceOpacity);
        }
    }

    material MetalPresetCopper_out = surface_copper_out;
    out1 = vec4(mx_srgb_encode(MetalPresetCopper_out.color), 1.0);
}
