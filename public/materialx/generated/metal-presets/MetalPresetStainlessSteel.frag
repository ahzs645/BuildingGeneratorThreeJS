...
uniform float conductor_weight;
uniform vec3 conductor_ior;
uniform vec3 conductor_extinction;
uniform vec2 conductor_roughness;
uniform float conductor_thinfilm_thickness;
uniform float conductor_thinfilm_ior;
uniform int conductor_distribution;
uniform float surface_stainless_steel_opacity;
uniform bool surface_stainless_steel_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_stainless_steel_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_stainless_steel_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            mx_conductor_bsdf(closureData, conductor_weight, conductor_ior, conductor_extinction, conductor_roughness, conductor_thinfilm_thickness, conductor_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, conductor_distribution, conductor_out);

            // Accumulate the light's contribution
            surface_stainless_steel_out.color += lightShader.intensity * conductor_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_stainless_steel_out.color += occlusion * conductor_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
        surface_stainless_steel_out.color += conductor_out.response;

        // Compute and apply surface opacity
        {
            surface_stainless_steel_out.color *= surfaceOpacity;
            surface_stainless_steel_out.transparency = mix(vec3(1.0), surface_stainless_steel_out.transparency, surfaceOpacity);
        }
    }

    material MetalPresetStainlessSteel_out = surface_stainless_steel_out;
    out1 = vec4(mx_srgb_encode(MetalPresetStainlessSteel_out.color), 1.0);
}
