...
uniform float surface_aluminum_opacity;
uniform bool surface_aluminum_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_aluminum_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_aluminum_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            surface_aluminum_out.color += lightShader.intensity * conductor_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_aluminum_out.color += occlusion * conductor_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
        surface_aluminum_out.color += conductor_out.response;

        // Compute and apply surface opacity
        {
            surface_aluminum_out.color *= surfaceOpacity;
            surface_aluminum_out.transparency = mix(vec3(1.0), surface_aluminum_out.transparency, surfaceOpacity);
        }
    }

    material MetalPresetAluminum_out = surface_aluminum_out;
    out1 = vec4(mx_srgb_encode(MetalPresetAluminum_out.color), 1.0);
}
