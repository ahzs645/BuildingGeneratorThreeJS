...
uniform float rotated_tangent_amount;
...
uniform float surface_f82_gold_anisotropic_r90_opacity;
uniform bool surface_f82_gold_anisotropic_r90_thin_walled;

in vec3 normalWorld;
in vec3 tangentObject;
...
void main()
{
    ...
    vec3 tangent_world_out = normalize(tangentObject);
    vec3 normal_world_out = normalize(normalObject);
    vec3 rotated_tangent_out = vec3(0.0);
    mx_rotate_vector3(tangent_world_out, rotated_tangent_amount, normal_world_out, rotated_tangent_out);
    vec3 normalized_tangent_out = normalize(rotated_tangent_out);
    surfaceshader surface_f82_gold_anisotropic_r90_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_f82_gold_anisotropic_r90_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            mx_generalized_schlick_bsdf(closureData, f82_weight, f82_color0, f82_color82, f82_color90, f82_exponent, f82_roughness, f82_thinfilm_thickness, f82_thinfilm_ior, geomprop_Nworld_out1, normalized_tangent_out, f82_distribution, f82_scatter_mode, f82_out);

            // Accumulate the light's contribution
            surface_f82_gold_anisotropic_r90_out.color += lightShader.intensity * f82_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_f82_gold_anisotropic_r90_out.color += occlusion * f82_out.response;
        }
        ...
        BSDF f82_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf(closureData, f82_weight, f82_color0, f82_color82, f82_color90, f82_exponent, f82_roughness, f82_thinfilm_thickness, f82_thinfilm_ior, geomprop_Nworld_out1, normalized_tangent_out, f82_distribution, f82_scatter_mode, f82_out);
        surface_f82_gold_anisotropic_r90_out.color += f82_out.response;

        // Compute and apply surface opacity
        {
            surface_f82_gold_anisotropic_r90_out.color *= surfaceOpacity;
            surface_f82_gold_anisotropic_r90_out.transparency = mix(vec3(1.0), surface_f82_gold_anisotropic_r90_out.transparency, surfaceOpacity);
        }
    }

    material MetalF82GoldAnisotropicR90_out = surface_f82_gold_anisotropic_r90_out;
    out1 = vec4(mx_srgb_encode(MetalF82GoldAnisotropicR90_out.color), 1.0);
}
