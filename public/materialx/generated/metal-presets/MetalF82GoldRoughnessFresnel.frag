...
uniform float surface_f82_gold_roughness_fresnel_opacity;
uniform bool surface_f82_gold_roughness_fresnel_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_f82_gold_roughness_fresnel_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_f82_gold_roughness_fresnel_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            surface_f82_gold_roughness_fresnel_out.color += lightShader.intensity * f82_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_f82_gold_roughness_fresnel_out.color += occlusion * f82_out.response;
        }
        ...
        BSDF f82_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf(closureData, f82_weight, f82_color0, f82_color82, f82_color90, f82_exponent, microfacet_alpha_xy_out, f82_thinfilm_thickness, f82_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, f82_distribution, f82_scatter_mode, f82_out);
        surface_f82_gold_roughness_fresnel_out.color += f82_out.response;

        // Compute and apply surface opacity
        {
            surface_f82_gold_roughness_fresnel_out.color *= surfaceOpacity;
            surface_f82_gold_roughness_fresnel_out.transparency = mix(vec3(1.0), surface_f82_gold_roughness_fresnel_out.transparency, surfaceOpacity);
        }
    }

    material MetalF82GoldRoughnessFresnel_out = surface_f82_gold_roughness_fresnel_out;
    out1 = vec4(mx_srgb_encode(MetalF82GoldRoughnessFresnel_out.color), 1.0);
}
