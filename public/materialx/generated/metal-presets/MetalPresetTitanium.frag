...
uniform float surface_titanium_opacity;
uniform bool surface_titanium_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_titanium_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_titanium_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            surface_titanium_out.color += lightShader.intensity * conductor_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_titanium_out.color += occlusion * conductor_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
        surface_titanium_out.color += conductor_out.response;

        // Compute and apply surface opacity
        {
            surface_titanium_out.color *= surfaceOpacity;
            surface_titanium_out.transparency = mix(vec3(1.0), surface_titanium_out.transparency, surfaceOpacity);
        }
    }

    material MetalPresetTitanium_out = surface_titanium_out;
    out1 = vec4(mx_srgb_encode(MetalPresetTitanium_out.color), 1.0);
}
