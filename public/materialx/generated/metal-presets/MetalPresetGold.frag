...
uniform float surface_gold_opacity;
uniform bool surface_gold_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_gold_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_gold_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            surface_gold_out.color += lightShader.intensity * conductor_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_gold_out.color += occlusion * conductor_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF conductor_out = BSDF(vec3(0.0),vec3(1.0));
        surface_gold_out.color += conductor_out.response;

        // Compute and apply surface opacity
        {
            surface_gold_out.color *= surfaceOpacity;
            surface_gold_out.transparency = mix(vec3(1.0), surface_gold_out.transparency, surfaceOpacity);
        }
    }

    material MetalPresetGold_out = surface_gold_out;
    out1 = vec4(mx_srgb_encode(MetalPresetGold_out.color), 1.0);
}
