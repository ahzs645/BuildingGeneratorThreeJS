...
uniform float roughness_025_weight;
uniform vec3 roughness_025_color0;
uniform vec3 roughness_025_color82;
uniform vec3 roughness_025_color90;
uniform float roughness_025_exponent;
uniform vec2 roughness_025_roughness;
uniform float roughness_025_thinfilm_thickness;
uniform float roughness_025_thinfilm_ior;
uniform int roughness_025_distribution;
uniform int roughness_025_scatter_mode;
uniform float roughness_050_weight;
uniform vec3 roughness_050_color0;
uniform vec3 roughness_050_color82;
uniform vec3 roughness_050_color90;
uniform float roughness_050_exponent;
uniform vec2 roughness_050_roughness;
uniform float roughness_050_thinfilm_thickness;
uniform float roughness_050_thinfilm_ior;
uniform int roughness_050_distribution;
uniform int roughness_050_scatter_mode;
uniform float roughness_075_weight;
uniform vec3 roughness_075_color0;
uniform vec3 roughness_075_color82;
uniform vec3 roughness_075_color90;
uniform float roughness_075_exponent;
uniform vec2 roughness_075_roughness;
uniform float roughness_075_thinfilm_thickness;
uniform float roughness_075_thinfilm_ior;
uniform int roughness_075_distribution;
uniform int roughness_075_scatter_mode;
uniform float roughness_100_weight;
uniform vec3 roughness_100_color0;
uniform vec3 roughness_100_color82;
uniform vec3 roughness_100_color90;
uniform float roughness_100_exponent;
uniform vec2 roughness_100_roughness;
uniform float roughness_100_thinfilm_thickness;
uniform float roughness_100_thinfilm_ior;
uniform int roughness_100_distribution;
uniform int roughness_100_scatter_mode;
uniform float mix_025_050_mix;
uniform float mix_050_075_mix;
uniform float mix_075_100_mix;
uniform float surface_f82_gold_layered_roughness_opacity;
uniform bool surface_f82_gold_layered_roughness_thin_walled;
...
void main()
{
    ...
    surfaceshader surface_f82_gold_layered_roughness_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = surface_f82_gold_layered_roughness_opacity;
        ...
        for (int activeLightIndex = 0; activeLightIndex < numLights; ++activeLightIndex)
        {
            ...
            BSDF roughness_100_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, roughness_100_weight, roughness_100_color0, roughness_100_color82, roughness_100_color90, roughness_100_exponent, roughness_100_roughness, roughness_100_thinfilm_thickness, roughness_100_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, roughness_100_distribution, roughness_100_scatter_mode, roughness_100_out);
            BSDF roughness_075_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, roughness_075_weight, roughness_075_color0, roughness_075_color82, roughness_075_color90, roughness_075_exponent, roughness_075_roughness, roughness_075_thinfilm_thickness, roughness_075_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, roughness_075_distribution, roughness_075_scatter_mode, roughness_075_out);
            BSDF roughness_050_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, roughness_050_weight, roughness_050_color0, roughness_050_color82, roughness_050_color90, roughness_050_exponent, roughness_050_roughness, roughness_050_thinfilm_thickness, roughness_050_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, roughness_050_distribution, roughness_050_scatter_mode, roughness_050_out);
            BSDF roughness_025_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, roughness_025_weight, roughness_025_color0, roughness_025_color82, roughness_025_color90, roughness_025_exponent, roughness_025_roughness, roughness_025_thinfilm_thickness, roughness_025_thinfilm_ior, geomprop_Nworld_out1, geomprop_Tworld_out1, roughness_025_distribution, roughness_025_scatter_mode, roughness_025_out);
            BSDF mix_025_050_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, roughness_050_out, roughness_025_out, mix_025_050_mix, mix_025_050_out);
            BSDF mix_050_075_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, roughness_075_out, mix_025_050_out, mix_050_075_mix, mix_050_075_out);
            BSDF mix_075_100_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, roughness_100_out, mix_050_075_out, mix_075_100_mix, mix_075_100_out);

            // Accumulate the light's contribution
            surface_f82_gold_layered_roughness_out.color += lightShader.intensity * mix_075_100_out.response;
        }
        ...
        // Add environment contribution
        {
            ...
            surface_f82_gold_layered_roughness_out.color += occlusion * mix_075_100_out.response;
        }
        ...
        BSDF mix_075_100_out = BSDF(vec3(0.0),vec3(1.0));
        mx_mix_bsdf(closureData, roughness_100_out, mix_050_075_out, mix_075_100_mix, mix_075_100_out);
        surface_f82_gold_layered_roughness_out.color += mix_075_100_out.response;

        // Compute and apply surface opacity
        {
            surface_f82_gold_layered_roughness_out.color *= surfaceOpacity;
            surface_f82_gold_layered_roughness_out.transparency = mix(vec3(1.0), surface_f82_gold_layered_roughness_out.transparency, surfaceOpacity);
        }
    }

    material MetalF82GoldLayeredRoughness_out = surface_f82_gold_layered_roughness_out;
    out1 = vec4(mx_srgb_encode(MetalF82GoldLayeredRoughness_out.color), 1.0);
}
