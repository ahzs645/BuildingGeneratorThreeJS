...
uniform float base_metal_weight;
uniform vec3 base_metal_ior;
uniform vec3 base_metal_extinction;
uniform vec2 base_metal_roughness;
uniform float base_metal_thinfilm_thickness;
uniform float base_metal_thinfilm_ior;
uniform int base_metal_distribution;
uniform float detail_metal_weight;
uniform vec3 detail_metal_ior;
uniform vec3 detail_metal_extinction;
uniform vec2 detail_metal_roughness;
uniform float detail_metal_thinfilm_thickness;
uniform float detail_metal_thinfilm_ior;
uniform int detail_metal_distribution;
uniform sampler2D layer_mask_file;
uniform int layer_mask_layer;
uniform vec3 layer_mask_default;
uniform int layer_mask_uaddressmode;
uniform int layer_mask_vaddressmode;
uniform int layer_mask_filtertype;
uniform int layer_mask_framerange;
uniform int layer_mask_frameoffset;
uniform int layer_mask_frameendaction;
uniform vec2 layer_mask_uv_scale;
uniform vec2 layer_mask_uv_offset;
uniform int layer_factor_index;
uniform float synthetic_surface_opacity;
uniform bool synthetic_surface_thin_walled;
...
vec3 mx_generate_prefilter_env()
{
    ...
    vec2 uv = gl_FragCoord.xy * exp2(float(u_envPrefilterMip)) / vec2(textureSize(u_envRadiance, 0));
...
void main()
{
    out1 = vec4(mx_generate_prefilter_env(), 1.0);
}
